...
def train_for_ur(epoch,epochs,model,opt,scheduler,train_dl,train_size):
    epoch_total_loss = 0
    epoch_loss0 = 0
    epoch_loss1 = 0
    epoch_loss2 = 0
...
def fit(model, train_dl, epochs=60, lr=1e-4,train_size = 384,mode = 'sal',save_dir = './loss.txt'):
    opt = get_opt(lr,model,mode)
    if(mode == 'sal'):
        scheduler = PolyLr(opt,gamma=0.9,minimum_lr=1.0e-07,max_iteration=len(train_dl)*epochs,warmup_iteration=12000)
    else:
        scheduler = PolyLr(opt,gamma=0.9,minimum_lr=1.0e-07,max_iteration=len(train_dl)*epochs,warmup_iteration=0)
    print('lr: '+str(lr))
...
def get_opt(lr,model,mode='sal'):
    if(mode=='sal'):
        ...
        ur_params = [name for name, params in model.named_parameters() if ("attention" in name)]
        ur_params = [params for name, params in model.named_parameters() if ("attention" in name)]
        
        params = [{'params': ur_params, 'lr': lr},
            ]
...
def train(args):
    ...
    fit(model,train_dl,args.train_epochs,args.lr,args.img_size,'sal',args.record)
    #model.mode = 'ur'
    #fit(model,train_dl,20,args.lr,args.img_size,'ur',args.record)
